...
namespace DS.Utilities
{
    ...
    public static class DSIOUtility
    {
        ...
        private static string SaveImageAsset(Texture2D image, string nodeName)
        {
            string imageFolder = $"{containerFolderPath}/Images";
            CreateFolder(containerFolderPath, "Images");

            string imagePath = $"{imageFolder}/{nodeName}_Image.png";
            System.IO.File.WriteAllBytes(imagePath, image.EncodeToPNG());

            AssetDatabase.ImportAsset(imagePath); // Import the asset to make it usable
            return AssetDatabase.AssetPathToGUID(imagePath); // Return the GUID of the image asset
        }
        ...
        private static void LoadNodes(List<DSNodeSaveData> nodes)
        {
            foreach (DSNodeSaveData nodeData in nodes)
            {
                ...
                node.ImagePath = nodeData.ImagePath;

                if (!string.IsNullOrEmpty(nodeData.ImagePath))
                {
                    string imageAssetPath = AssetDatabase.GUIDToAssetPath(nodeData.ImagePath);
                    Texture2D image = AssetDatabase.LoadAssetAtPath<Texture2D>(imageAssetPath);
                    if (image != null)
                    {
                        node.SetImage(image); // Make sure SetImage method is available in DSNode
                    }
                }
